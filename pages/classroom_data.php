...
<body class="g-sidenav-show  bg-gray-200">
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
    ...
    <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link text-white active bg-gradient-info " href="classroom_data.php">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
        ...
        <div class="col-12">
          <div class="card my-4">
            ...
            <div class="bg-gradient-info shadow-info border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center">
              <h6 class="text-white text-capitalize ps-3 mb-0">Classroom Data</h6>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">No</th>
                      ...
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">Teacher Name</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                  <?php
                  $id = 1; // Inisialisasi variabel $id dengan nilai awal 1.
                  
                  // Melakukan query SQL untuk mengambil data kelas beserta informasi mata pelajaran dan guru yang terkait.
                  $tampil = mysqli_query($koneksi, "SELECT tb_kelas.id, tb_mapel.nama_mata_pelajaran, tb_guru.nama_guru, tb_kelas.id_mata_pelajaran, tb_kelas.id_guru
                                                    FROM tb_kelas
                                                    INNER JOIN tb_mapel ON tb_kelas.id_mata_pelajaran = tb_mapel.id
                                                    INNER JOIN tb_guru ON tb_kelas.id_guru = tb_guru.id
                                                    ORDER BY tb_kelas.id
                                                    ");
                  ...
                  <tr>
                    ...
                    <td class="align-middle text-center">
                      <div class="d-flex flex-column justify-content-center">
                        <h6 class="mb-0 text-sm"><?= $data['nama_guru']?></h6>
                      </div>
                    </td>
                    <td class="align-middle text-center">
                      ...
                      <div class="modal-dialog">
                          <div class="modal-content">
                              ...
                              <div class="modal-body">
                                  <form method="POST" action="../crud/create_classroom_data.php" role="form" onsubmit="return validateForm()">
                                      ...
                                      <div class="input-group input-group-outline mb-3">
                                          <select name="id_guru" class="form-control">
                                              ...
                                              <?php
                                              require_once('../function/koneksi.php');
                                              $query = "SELECT id, nama_guru FROM tb_guru";
                                              $result = mysqli_query($koneksi, $query);
                                              while ($row = mysqli_fetch_assoc($result)) {
                                                  echo '<option value="' . $row['id'] . '">' . $row['nama_guru'] . '</option>';
                                              }
                                              ?>
                                          </select>
                                      </div>                                
                  ...
                  <div class="modal fade" id="ModalUbah<?= $id ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog">
                          <div class="modal-content">
                              ...
                              <div class="modal-body">
                                  <form method="POST" action="../crud/edit_classroom_data.php" role="form">
                                      ...
                                      <div class="input-group input-group-outline mb-3">
                                          <select name="id_mata_pelajaran" class="form-control">
                                              ...
                                              $result = mysqli_query($koneksi, $query);
                                              while ($row = mysqli_fetch_assoc($result)) {
                                                  $selected = ($row['id'] == $data['id_mata_pelajaran']) ? 'selected' : '';
                                      ...
                                      <div class="input-group input-group-outline mb-3">
                                          <select name="id_guru" class="form-control">
                                              ...
                                              $result = mysqli_query($koneksi, $query);
                                              while ($row = mysqli_fetch_assoc($result)) {
                                                  $selected = ($row['id'] == $data['id_guru']) ? 'selected' : '';
                                                  echo '<option value="' . $row['id'] . '" ' . $selected . '>' . $row['nama_guru'] . '</option>';
                                              }
                                              ?>
                                          </select>
                                      </div>
                  ...
                  <div class="modal fade" id="ModalHapus<?= $id ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" >
                      <div class="modal-dialog">
                          <div class="modal-content">   
                              ...
                              <div class="modal-body">
                                  <form method="POST" action="../crud/delete_classroom_data.php" role="form">
                                      <input type="hidden" name="id" value="<?= $data['id']?>" >
...
function validateForm() {
    var subjectSelect = document.querySelector('select[name="id_mata_pelajaran"]');
    var teacherSelect = document.querySelector('select[name="id_guru"]');

    if (subjectSelect.value === "Choose Subjects" || teacherSelect.value === "Choose Guru") {
        alert("Please select a subject and a teacher before submitting the form.");
        return false;
    }
...
function resetForm() {
    document.querySelector('select[name="id_mata_pelajaran"]').value = "Choose Subjects";
    document.querySelector('select[name="id_guru"]').value = "Choose Guru";
}
</script>
